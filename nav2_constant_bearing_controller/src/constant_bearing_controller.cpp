...
#include "nav2_constant_bearing_controller/constant_bearing_controller.hpp"
...
namespace nav2_constant_bearing_controller
{

void ConstantBearingController::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  //new
  auto plan_center =geometry_msgs::msg::PoseStamped();
  ...
  //new
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>("goal_pose",1, std::bind(&ConstantBearingController::goal_callback, this, std::placeholders::_1) );
  //
...
void ConstantBearingController::cleanup()
{
  RCLCPP_INFO(
    logger_,
    "Cleaning up controller: %s of type"
    " constant_bearing_controller::ConstantBearingController",
    plugin_name_.c_str());
...
void ConstantBearingController::activate()
{
  RCLCPP_INFO(
    logger_,
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &ConstantBearingController::dynamicParametersCallback,
      this, std::placeholders::_1));
}

void ConstantBearingController::deactivate()
{
  RCLCPP_INFO(
    logger_,
...
void ConstantBearingController::goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg){
  plan_center.pose.position = msg->pose.position;
  plan_center.pose.orientation = msg->pose.orientation;
}

std::unique_ptr<geometry_msgs::msg::PointStamped> ConstantBearingController::createCarrotMsg(
...
double ConstantBearingController::getLookAheadDistance(
...
geometry_msgs::msg::TwistStamped ConstantBearingController::computeVelocityCommands(
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  //double bearing_to_lookahead = atan2(carrot_pose.pose.position.y, carrot_pose.pose.position.x);
  double relative_x = pose.pose.position.x - plan_center.pose.position.x;
  double relative_y = pose.pose.position.y - plan_center.pose.position.y;
  double tangent_angle = atan2(relative_y, relative_x) + M_PI/2;
  ...
  if (shouldRotateToGoalHeading(carrot_pose)) {
    ...
    double K = 0.5;//gain
    double heading_error = tangent_angle - current_heading;
    heading_error = atan2(sin(heading_error), cos(heading_error)); //map to -pi to pi
    angular_vel = K*heading_error;
  }

  // Collision checking on this velocity heading
  const double & carrot_dist = hypot(carrot_pose.pose.position.x, carrot_pose.pose.position.y);
  if (use_collision_detection_ && isCollisionImminent(pose, linear_vel, angular_vel, carrot_dist)) {
    throw nav2_core::PlannerException("ConstantBearingController detected collision ahead!");
  }
...
bool ConstantBearingController::shouldRotateToPath(
...
bool ConstantBearingController::shouldRotateToGoalHeading(
  const geometry_msgs::msg::PoseStamped & carrot_pose)
{
...
void ConstantBearingController::rotateToHeading(
...
geometry_msgs::msg::PoseStamped ConstantBearingController::getLookAheadPoint(
...
bool ConstantBearingController::isCollisionImminent(
...
bool ConstantBearingController::inCollision(
  const double & x,
  const double & y,
  const double & theta)
{
...
double ConstantBearingController::costAtPose(const double & x, const double & y)
{
  unsigned int mx, my;
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    ...
    throw nav2_core::PlannerException(
            "ConstantBearingController: Dimensions of the costmap are too small "
...
double ConstantBearingController::approachVelocityScalingFactor(
...
void ConstantBearingController::applyApproachVelocityScaling(
...
void ConstantBearingController::applyConstraints(
...
void ConstantBearingController::setPlan(const nav_msgs::msg::Path & path)
{
  global_plan_ = path;
}

void ConstantBearingController::setSpeedLimit(
...
nav_msgs::msg::Path ConstantBearingController::transformGlobalPlan(
  const geometry_msgs::msg::PoseStamped & pose)
{
...
double ConstantBearingController::findVelocitySignChange(
  const nav_msgs::msg::Path & transformed_plan)
{
...
bool ConstantBearingController::transformPose(
...
double ConstantBearingController::getCostmapMaxExtent() const
{
...
PLUGINLIB_EXPORT_CLASS(
  nav2_constant_bearing_controller::ConstantBearingController,
